...
def build_context_abstracts_only(topic: str, hits: list[dict], max_items: int = 5, max_abs_chars: int = 1200) -> str:
    lines = [
        f"ROUTED_TOPIC: {topic}",
        "EVIDENCE (ABSTRACTS ONLY):",
        "Use ONLY the evidence below for medical claims. If missing, say 'insufficient evidence'.",
    ]

    for i, h in enumerate(hits[:max_items], start=1):
        abs_snip = (h.get("abstract") or "").strip()
        if len(abs_snip) > max_abs_chars:
            abs_snip = abs_snip[:max_abs_chars].rstrip() + "..."

        lines.append(f"\n[{i}] id={h.get('id')} score={h.get('score'):.3f}")
        lines.append(f"TITLE: {h.get('title','').strip()}")
        lines.append(f"ABSTRACT: {abs_snip if abs_snip else '(no abstract)'}")

    return "\n".join(lines)

context = build_context_abstracts_only(best_topic["topic"], hits, max_items=5, max_abs_chars=1200)
print(context)
